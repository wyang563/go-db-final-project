...
func (a *Aggregator) Descriptor() *TupleDesc {
	mergedStateDesc := &TupleDesc{}
	var groupByFields []FieldType
	for _, aggState := range a.newAggState {
		mergedStateDesc = mergedStateDesc.merge(aggState.GetTupleDesc())
	}
	// if len(a.groupByFields) == 0 {
	// 	return mergedStateDesc;
	// } else {
	// 	for _, groupExpr := range a.groupByFields {
	// 		groupByFields = append(groupByFields, groupExpr.GetExprType());
	// 	}
	// 	groupDesc := TupleDesc{Fields: groupByFields};
	// 	return groupDesc.merge(mergedStateDesc);
	// }
	for _, groupExpr := range a.groupByFields {
		groupByFields = append(groupByFields, groupExpr.GetExprType())
	}
...
func extractGroupByKeyTuple(a *Aggregator, t *Tuple) (*Tuple, error) {
	resDesc := *(a.Descriptor())
	var resFields []DBValue
	for _, groupExpr := range a.groupByFields {
		groupVal, err := groupExpr.EvalExpr(t)
		if err != nil {
			return nil, err
		}
		resFields = append(resFields, groupVal)
	}
	return &Tuple{Desc: resDesc, Fields: resFields}, nil
}
...
func addTupleToGrpAggState(a *Aggregator, t *Tuple, grpAggState *[]AggState) {
	for i, aggState := range *(grpAggState) {
		// init aggState if it is nil
		if aggState == nil {
			(*grpAggState)[i] = a.newAggState[i].Copy()
		}
		// add elem to grpAggState index i
...
func getFinalizedTuplesIterator(a *Aggregator, groupByList []*Tuple, aggState map[any]*[]AggState) func() (*Tuple, error) {
	...
	// retDesc := a.Descriptor();
	return func() (*Tuple, error) {
		// check if we've reached end of index list
		if curGbyTuple >= len(groupByList) {
			return nil, nil
		}
		groupByTuple := groupByList[curGbyTuple]
		key := groupByTuple.tupleKey()
		// get key assosciated with tuple
		aggVal := (*aggState[key])[0].Finalize()
		res := joinTuples(groupByTuple, aggVal)
		res.Desc = *(*a).Descriptor();
		curGbyTuple++
		return res, nil
	}
}
