...
func (td *TupleDesc) copy() *TupleDesc {
	...
	copyVal := make([]FieldType, len(td.Fields))
	copy(copyVal, td.Fields)
	return &TupleDesc{copyVal} //replace me
}
...
func (desc *TupleDesc) merge(desc2 *TupleDesc) *TupleDesc {
	// TODO: some code goes here
	left := desc.copy().Fields
	right := desc2.copy().Fields
	mergeVal := append(left, right...)
	return &TupleDesc{mergeVal} //replace me
}
...
func (t *Tuple) writeTo(b *bytes.Buffer) error {
	...
	for _, f := range t.Fields {
		switch f := f.(type) {
		case StringField:
			writeVal := make([]byte, StringLength)
			copy(writeVal, f.Value)
			err := binary.Write(b, binary.LittleEndian, writeVal)
			if err != nil {
...
func readTupleFrom(b *bytes.Buffer, desc *TupleDesc) (*Tuple, error) {
	...
	for i, f := range desc.Fields {
		...
		case IntType:
			var readVal int64
			err := binary.Read(b, binary.LittleEndian, &readVal)
			if err != nil {
				fmt.Println("binary.Read failed:", err)
			}
			fields[i] = IntField{readVal}
		case StringType:
			readVal := make([]byte, StringLength)
			err := binary.Read(b, binary.LittleEndian, &readVal)
			if err != nil {
				fmt.Println("binary.Read failed:", err)
			}
			finalString := ""
			// cut out ending empty chars
			for i := 0; i < len(readVal); i++ {
				if readVal[i] != 0 {
					finalString = finalString + string(readVal[i])
				} else {
					break
				}
			}
			fields[i] = StringField{finalString}
		}
	}
	return &Tuple{Fields: fields, Desc: *desc}, nil
}
...
func (t1 *Tuple) equals(t2 *Tuple) bool {
	// TODO: some code goes here
	descEqual := t1.Desc.equals(&t2.Desc)
	fieldEqual := true
...
func joinTuples(t1 *Tuple, t2 *Tuple) *Tuple {
	...
	mergeFields := make([]DBValue, len(t1.Fields)+len(t2.Fields))
	mergeDesc := t1.Desc.merge(&t2.Desc);
	copy(mergeFields, t1.Fields)
	for i := len(t1.Fields); i < len(mergeFields); i++ {
		mergeFields[i] = t2.Fields[i-len(t1.Fields)]
	}
	return &Tuple{Fields: mergeFields, Desc: *mergeDesc, Rid: t1.Rid}
}
...
func (t *Tuple) compareField(t2 *Tuple, field Expr) (orderByState, error) {
	// TODO: some code goes here
	tRes, err := field.EvalExpr(t)
	var res orderByState
	// cast to specific type
	switch tRes := tRes.(type) {
	case IntField:
		t2Res0, _ := field.EvalExpr(t2)
		t2Res := t2Res0.(IntField)
		tVal := tRes.Value
		t2Val := t2Res.Value
		if tVal < t2Val {
			res = OrderedLessThan
			...
			res = OrderedGreaterThan
		}
	case StringField:
		t2Res0, _ := field.EvalExpr(t2)
		t2Res := t2Res0.(StringField)
		tVal := tRes.Value
		t2Val := t2Res.Value
		if tVal < t2Val {
			res = OrderedLessThan
...
func (t *Tuple) project(fields []FieldType) (*Tuple, error) {
	// TODO: some code goes here
	// var fieldSet = make(map[string]int)
	// for _, k := range fields {
	// 	fieldSet[k.Fname] = 0
	// }
	var filterDesc []FieldType
	var filterFields []DBValue
	for i := 0; i < len(fields); i++ {
		ind, err := findFieldInTd(fields[i], &t.Desc);
		if err != nil {
			return nil, err;
		}
		filterDesc = append(filterDesc, fields[i]);
		filterFields = append(filterFields, t.Fields[ind]);
	}
	newDesc := TupleDesc{Fields: filterDesc}
	return &Tuple{Desc: newDesc, Fields: filterFields, Rid: t.Rid}, nil //replace me
}
