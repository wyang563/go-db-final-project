...
func (hj *EqualityJoin[T]) Descriptor() *TupleDesc {
	...
	leftOp := *(hj.left);
	rightOp := *(hj.right);
	leftDesc := leftOp.Descriptor();
	rightDesc := rightOp.Descriptor();
...
func (joinOp *EqualityJoin[T]) Iterator(tid TransactionID) (func() (*Tuple, error), error) {
	outerIter, err := (*(joinOp.left)).Iterator(tid);
	if err != nil {
		fmt.Println("Error Getting Outer Iter");
		return nil, err;
	}
	innerIter, err := (*(joinOp.right)).Iterator(tid);
	if err != nil {
		fmt.Println("Error Getting Inner Iter");
		return nil, err;
	}
	outTup, _ := outerIter();
	return func() (*Tuple, error) {
		for outTup != nil {
			inTup, _ := innerIter();
			for inTup != nil {
				inVal, _ := joinOp.rightField.EvalExpr(inTup);
				outVal, _ := joinOp.leftField.EvalExpr(outTup);
				match := false;
				switch inVal.(type) {
				case IntField:
					inOut := inVal.(IntField);
					innerField := inOut.Value;
					outOut := outVal.(IntField);
					outField := outOut.Value;
					if innerField == outField {
						match = true;
					}
				case StringField:
					inOut := inVal.(StringField);
					innerField := inOut.Value;
					outOut := outVal.(StringField);
					outField := outOut.Value;
					if innerField == outField {
						match = true;
					}
				...
				if match {
					return joinTuples(outTup, inTup), nil;
				}
				inTup, _ = innerIter();
			}
			// reset inner iterator if inTup is empty
			innerIter, err = (*(joinOp.right)).Iterator(tid);
			if err != nil {
				fmt.Println("Error in Getting Outer Iter");
				return nil, err;
			}
			outTup, _ = outerIter();
			if outTup == nil {
