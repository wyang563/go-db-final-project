...
func (a *SumAggState[T]) AddTuple(t *Tuple) {
	...
	if err != nil {
		fmt.Println("error in evaluating tuple in Sum Agg");
		return;
	}
	switch v.(type) {
	case IntField:
		a.sum += intAggGetter(v).(int64);
	}
}
...
func (a *AvgAggState[T]) Copy() AggState {
	return &AvgAggState[T]{a.alias, a.expr, a.avg, a.sum, a.count, a.getter};
}
...
func (a *AvgAggState[T]) AddTuple(t *Tuple) {
	v, err := a.expr.EvalExpr(t);
	if err != nil {
		fmt.Println("error in evaluating tuple in Avg Agg");
		return;
	}
	switch v.(type) {
	case IntField:
